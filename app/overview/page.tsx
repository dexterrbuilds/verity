...
export default async function OverviewPage() {
  const { data, stats, topForecasters, trendingMarkets, recentResolved, insights } = await getOverviewData();
  ...
  return (
    <section className="container-page py-10">
      <div className="flex flex-col gap-2">
        ...
        <p className="max-w-3xl text-muted-foreground">{demo ? "A seeded demo view of important markets, fictional forecasters, and reputation-weighted conviction." : "An early-access view of tracked markets, forecasters, and reputation-weighted conviction."}</p>
      </div>
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-4">
        ...
        <StatCard label="Forecasters" value={String(stats.totalForecasters)} detail={demo ? "fictional demo identities" : "tracked identities"} icon={<Award className="h-5 w-5" />} />
        ...
        <StatCard label="Tracked forecasts" value={String(stats.totalForecasts)} detail={demo ? "seeded for validation" : "persisted records"} icon={<Activity className="h-5 w-5" />} />
      </div>
