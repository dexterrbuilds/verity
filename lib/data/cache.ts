...
export const getConnectedBaseData = cache(async () => {
  ...
  const [categories, protocols, markets, forecasters, forecasts] = await Promise.all([
    ...
    supabase.from("markets").select("id,protocol_id,category_id,slug,question,description,source_url,current_probability,previous_probability,volume,participant_count,resolution_date,resolution_status,resolution_outcome,resolution_rules,data_origin,verification_status,created_at,updated_at").order("created_at", { ascending: false }).limit(500),
