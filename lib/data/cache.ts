...
export const getConnectedBaseData = cache(async () => {
  ...
  const [categories, protocols, markets, forecasters, forecasts] = await Promise.all([
    ...
    supabase.from("protocols").select("id,name,slug,logo_url,website_url,description,created_at,updated_at").order("name"),
    supabase.from("markets").select("id,protocol_id,category_id,slug,question,description,source_url,current_probability,previous_probability,volume,participant_count,resolution_date,resolution_status,resolution_outcome,resolution_rules,created_at,updated_at").order("created_at", { ascending: false }).limit(500),
    supabase.from("forecasters").select("id,slug,display_name,wallet_address,x_handle,avatar_url,bio,joined_at,is_verified,created_at,updated_at").order("display_name").limit(500),
    supabase.from("forecasts").select("id,forecaster_id,market_id,predicted_probability,confidence,position,reasoning,forecasted_at,is_resolved,was_correct,score_impact,created_at,updated_at").order("forecasted_at", { ascending: false }).limit(5000)
  ]);
